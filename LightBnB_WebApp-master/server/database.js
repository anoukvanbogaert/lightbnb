...
const addUser = (user) => {
  ...
  return pool
    .query(`INSERT INTO users (name, email, password) VALUES ($1, $2, $3) RETURNING *`, Object.values(user))
    .then((res) => {
      console.log(res.rows[0]); // just index so it doesn't return the array incl obj
      return res.rows[0];
    })
    .catch((err) => {
      console.log(err.message);
...
const getAllReservations = (guest_id) => {
  ...
  return pool
    ...
    LIMIT 10;`, [guest_id])
    .then((res) => {
      // console.log('result:',res); // just index so it doesn't return the array incl obj
      return res.rows;
    })
    .catch((err) => {
      console.log(err.message);
...
const getAllProperties = (options, limit = 10) => {
  ...
  let queryString = `
    ...
    `;
  if (options.city) { // if there's a filter for city, then
